...
import {
    ...
    findTransactionSubmissionChangePaymentGateway,
} from '../actions/get'
import {
...
import {
    loadMoreTransactionSubmissionPaid
...
const TransactionSubmissionChangePaymentGatewayDashboard = () => {
    ...
    const {resetErrorTransctionSubmissionPaid, setCurrentPage} = transactionSubmissionPaidSlice.actions
    const {
        ...
        page,
        currentPage,
        totalRecordTransactionSubmissionPaid,
        hasMore,
    ...
    const loadMoreTransactionSubmissionPaidCallback = useCallback(() => {
        dispatch(loadMoreTransactionSubmissionPaid())
    }, [dispatch])

    const handlePageTransactionPaidInc = (value) => {
        if (value > page && hasMore) {
            loadMoreTransactionSubmissionPaidCallback()
    ...
    const handleRefresh = () => {
        if (activeTab === 'pending') {
            ...
            dispatch(fetchTransactionSubmissionPaid(1, false))
    ...
    const {resetFindTransactionSubmissionChangePaymentGateway} = findTransactionSubmissionChangePaymentGatewaySlice.actions
    const {
        dataFindTransactionSubmissionChangePaymentGateway, 
        errorFindTransactionSubmissionChangePaymentGateway,
    ...
    useEffect(() => {
        if (errorFindTransactionSubmissionChangePaymentGateway) {
            setError({
                type: 'error',
                message: errorFindTransactionSubmissionChangePaymentGateway,
            })
        }
    }, [errorFindTransactionSubmissionChangePaymentGateway])
    ...
    useEffect(() => {
        if (query === '') {
            dispatch(resetFindTransactionSubmissionChangePaymentGateway())
        }
    }, [query])

    const handleSearchTransaction = (key) => {
        setQuery(key)
        dispatch(findTransactionSubmissionChangePaymentGateway(key))
    }
                    ...
                    <div className="max-w-7xl mx-auto" style={{marginTop: headerHeight}}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <p className="text-2xl font-bold">{totalRecordTransactionSubmissionPaid}</p>
                                </div>
                        ...
                        <div className="relative mb-6 w-full">
                            ...
                            <input
                                type="text"
                                placeholder="Search by transaction ID, Xendit ID, or Tenant ID..."
                                value={query}
                        ...
                        <div className="border-b border-gray-200">
                            ...
                            <button
                                ...
                                (dataFindTransactionSubmissionChangePaymentGateway ? dataFindTransactionSubmissionChangePaymentGateway?.status === 'pending' : activeTab === 'pending')
                            ...
                            <button
                                ...
                                className={`py-4 px-1 border-b-2 font-medium text-sm transition-colors ${
                                (dataFindTransactionSubmissionChangePaymentGateway ? dataFindTransactionSubmissionChangePaymentGateway?.status === 'paid' : activeTab === 'paid')
                                    ? 'border-gray-900 text-gray-900'
                                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                                }`}
                            >
                                ...
                                Paid Transactions ({totalRecordTransactionSubmissionPaid})
                                </div>
                            </button>
                            </nav>
                        </div>
                            ...
                            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                                <div className="overflow-x-auto">
                                    <table className="w-full">
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {dataFindTransactionSubmissionChangePaymentGateway.map((transaction) => (
                        ...
                        {(activeTab === 'pending' && !dataFindTransactionSubmissionChangePaymentGateway) && (
                        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                        ...
                        {(activeTab === 'paid' && !dataFindTransactionSubmissionChangePaymentGateway) && (
                        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                            <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                            ...
                            <table className="w-full">
                                ...
                                {loadingTransactionSubmissionPaid ? (
                                    ...
                                    paidTransactions[currentPage-1]?.map((transaction, index) => (
                                    <tr key={transaction.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                                        <td className="px-6 py-4">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                            ...
                            {!loadingTransactionSubmissionPaid && paidTransactions.length === 0 && (
                            <div className="text-center py-8">
                        ...
                        { (activeTab == 'paid' && !dataFindTransactionSubmissionChangePaymentGateway) && (
                            <div className="mt-6 flex items-center justify-between">
                                <div className="flex items-center text-sm text-gray-700">
                                    Showing {paidTransactions[currentPage-1]?.length} results
                                </div>
                                <div className="flex items-center gap-2">
                                    <button 
                                    ...
                                    <button 
                                    disabled={(!hasMore && currentPage === paidTransactions.length) || loadingTransactionSubmissionPaid}
